...
class Card {
    ...
    static function getCard($name) {
        $data = static::getData($name);
        return new Card(
            static::getCardName($data),
            static::getManaCost($data),
            static::getCardType($data),
            static::getCardText($data),
            static::getPowerToughness($data),
            static::getLoyalty($data),
            static::getManaCostImage($data),
            static::getId($data)
            );
    }

    function __construct($name, $manacost, $type, $text, $pt, $loyalty, $manaimage, $id) {
        // Store data.
        $this->name = $name;
        ...
        $this->color = static::getColor($manacost);
        $this->image = static::getImage($name);
        $this->id = $id;
    }
