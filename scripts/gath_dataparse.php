...
// Fetch data.
function GetCardDataFromName($name) {
...
function GetCardArrayFromData($data) {
    $cardarray = array(
    "name" => GetCardNameFromData($data),
    "manacost" => GetManaCostFromData($data),
    "type" => GetCardTypeFromData($data),
    "text" => GetCardTextFromData($data),
    "pt" => GetPowerToughnessFromData($data),
    "loyalty" => GetLoyaltyFromData($data),
    );
    $cardarray["manaimage"] = GetManaCostImageFromData($data);
    $cardarray["color"] = GetColorFromManaCost($cardarray["manacost"]);
    $cardarray["image"] = getImage($cardarray["name"]);
    return $cardarray;
}

// Get card information from data.

function GetCardInfoFromData($data, $prefix, $suffix) {
    // NAME:    INFO_NAME_PREFIX, INFO_NAME_SUFFIX
    $position = strpos($data, $prefix);
    if ($position) {
        $full = substr($data, $position);           // String that starts with $prefix.
        $position = strpos($full, INFO_START_PATH); // Position of a global data prefix for internal data.
        $start = substr($full, $position + 21);     // String starting from end of global data prefix, and start of internal data.
        $endpos = strpos($start, $suffix);          // Length of internal data.
        
        $info = substr($start, 0, $endpos);         // String starting from internal data, at length of internal data.
        return trim($info);
    }
    return "";
}

function GetCardNameFromData($data) {
...
function GetManaCostFromData($data) {
...
function GetManaCostImageFromData($data) {
    $position = strpos($data, INFO_MANA_PREFIX);
    if ($position) {
        $start = substr($data, $position + 91); // String that starts with Mana Cost.
        $endpos = strpos($start, INFO_SUFFIX); // Length of Mana Cost.
...
function GetCardTypeFromData($data) {
...
function GetCardTextFromData($data) {
    ...
    if ($position) {
        ...
        return trim($cardtext . "</div>");
    }
    return "";
}

function GetPowerToughnessFromData($data) {
...
function GetLoyaltyFromData($data) {
